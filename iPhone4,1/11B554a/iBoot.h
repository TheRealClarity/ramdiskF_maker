#define IMAGE_NAME              "iBoot.n94ap.7.0.4.dec"
...
if (1) {
    ...
    switch (seq) {
        ...
        case 2:
            ...
            PUT_DWORD_LE(buffer, 0x46D48 + 100 - 0x46CD4, 0x41238 /* go command handler */);
            PUT_DWORD_LE(buffer, 0x46D48 + 104 - 0x46CD4, 0x1A328 /* allow unsigned images */);
...
{
    eprintf("irq_disable_count = 0x%x\n", *(unsigned int *)(image + 0x54940 + 0x18)); /**fix **/
}
...
dispatch_SEGV(void *si_addr, _STRUCT_ARM_THREAD_STATE *tctx)
{
    ...
    if (si_addr == 0) {
        if (tctx->__pc == (uintptr_t)(image + 0x20706)) {
            /* idle task crap (read from *0) */
            tctx->__r[0] = *(uint32_t *)(image);
            tctx->__pc += 2;
            return 0;
        }
        if (tctx->__pc == (uintptr_t)(image + 0x20966)) {
...
patch_image(unsigned char *image)
{
    ...
    *(uint32_t *)(image + 0x202D0) = INSN2_LDR_R1_PC__B_PLUS4;
    *(void **)(image + 0x202D4) = XLAT(IMAGE_END + IMAGE_HEAP_SIZE);
    /* clean data cache */
    *(uint32_t *)(image + 0x218BC) = INSNA_RETURN;
#if !USE_SIGNAL
    /* idle task crap (read from *0) */
    *(uint16_t *)(image + 0x20706) = INSNT_NOP;
    *(uint16_t *)(image + 0x20966) = INSNT_MOV_R_R(1, 0);
#endif /* !USE_SIGNAL */
    /* timer 2 */
    *(uint32_t *)(image + 0x1FA7C) = INSN2_RETURN_0;
    /* task switch FPU */
    *(uint32_t *)(image + 0x21CEC) = INSNA_MOV_R2_0;
    *(uint32_t *)(image + 0x21D1C) = INSNA_MOV_R2_0;
    /* bzero during mmu_init */
    *(uint32_t *)(image + 0x20EA0) = INSN2_NOP__NOP;
    ...
    *(uint32_t *)(image + 0x192E6) = INSN2_MOV_R0_1__MOV_R0_1;

    /* make main_task show SP */
    *(uint16_t *)(image + 0xD94) = INSNT_MOV_R_R(1, 13);
    *(uint8_t *)(image + 0x344A4) = 'x';
    /* show task structure */
    *(void **)(image + 0xFA4) = image + 0x43560;
    *(uint8_t *)(image + 0x34490) = 'x';

    /* nop some more hw */
    *(uint32_t *)(image + 0x0BB4C) = INSN2_RETURN_0;
    *(uint32_t *)(image + 0x01768) = INSN2_RETURN_0;
    *(uint32_t *)(image + 0x026D0) = INSN2_RETURN_0;
    *(uint32_t *)(image + 0x00D60) = INSN2_NOP__NOP;
    *(uint16_t *)(image + 0x0C138) = INSNT_NOP; /* XXX loop */
    *(uint16_t *)(image + 0x1FC36) = INSNT_NOP;
    *(uint32_t *)(image + 0x1EF98) = INSN2_RETURN_0;
    *(uint32_t *)(image + 0x1F1A6) = INSN2_RETURN_0;
    *(uint32_t *)(image + 0x1F1C6) = INSN2_RETURN_0;
    *(uint32_t *)(image + 0x1F200) = INSN2_RETURN_0;
    *(uint32_t *)(image + 0x1F218) = INSN2_RETURN_21;
    *(uint32_t *)(image + 0x1F230) = INSN2_RETURN_0;
}
